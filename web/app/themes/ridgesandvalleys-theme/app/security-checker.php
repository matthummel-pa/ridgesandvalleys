...
function rv_rest_security(\WP_REST_Request $req)
{
    ...
    foreach ($dom->getElementsByTagName('form') as $f) {
        if (stripos($f->getAttribute('action'), 'http://') === 0) {
            $insecureForms++;
        }
    }
    ...
    foreach ($xp->query('//a[@target="_blank"]') as $a) {
        ...
        if (strpos($rel, 'noopener') === false && strpos($rel, 'noreferrer') === false) {
            $tabnab++;
        }
    }
    ...
    $add(
        'https',
        __('http redirects to https', 'sage'),
        $redirect === true ? 'pass' : ($redirect === null ? 'warn' : 'fail'),
        8,
    ...
    $add(
        'https',
        __('Forms submit securely', 'sage'),
        $insecureForms === 0 ? 'pass' : 'fail',
        6,
    ...
    $add(
        'headers',
        __('Content-Security-Policy', 'sage'),
        $csp !== '' ? 'pass' : 'warn',
        8,
    ...
    $add(
        'headers',
        __('X-Content-Type-Options', 'sage'),
        $nosniff ? 'pass' : 'warn',
        5,
    ...
    $add(
        'disclosure',
        __('Server software version hidden', 'sage'),
        $serverLeaks ? 'warn' : 'pass',
        5,
    ...
    $add(
        'disclosure',
        __('No X-Powered-By header', 'sage'),
        $poweredBy === '' ? 'pass' : 'warn',
        4,
    ...
    $add(
        'disclosure',
        __('CMS version not advertised', 'sage'),
        $generator === '' ? 'pass' : 'warn',
        4,
    ...
    if ($hasCookies) {
        $add(
            'cookies',
            __('Secure flag on cookies', 'sage'),
            $ckSecure ? 'pass' : 'warn',
            6,
        ...
        $add(
            'cookies',
            __('HttpOnly flag on cookies', 'sage'),
            $ckHttpOnly ? 'pass' : 'warn',
            5,
        ...
        $add(
            'cookies',
            __('SameSite attribute', 'sage'),
            $ckSameSite ? 'pass' : 'warn',
            4,
        ...
        $add(
            'cookies',
            __('No cookies set on this page', 'sage'),
            'pass',
            4,
    ...
    $add(
        'frontend',
        __('External links are safe', 'sage'),
        $tabnab === 0 ? 'pass' : ($tabnab <= 3 ? 'warn' : 'fail'),
        6,
    ...
    $add(
        'frontend',
        __('Few inline event handlers', 'sage'),
        $inlineHandlers === 0 ? 'pass' : ($inlineHandlers <= 8 ? 'warn' : 'fail'),
        5,
    ...
    $add(
        'frontend',
        __('Limited inline scripts', 'sage'),
        $inlineScripts <= 3 ? 'pass' : ($inlineScripts <= 8 ? 'warn' : 'fail'),
        4,
    ...
    $add(
        'frontend',
        __('Third-party script sprawl', 'sage'),
        $thirdPartyCount <= 4 ? 'pass' : ($thirdPartyCount <= 8 ? 'warn' : 'fail'),
        5,
