...
function rv_rest_local(\WP_REST_Request $req)
{
    ...
    foreach ($xp->query('//a') as $a) {
        ...
        if (preg_match('#(g\.page|business\.google|goo\.gl/maps|maps\.app\.goo\.gl|google\.[a-z.]+/maps)#', $href)) {
            $gbpLink = true;
        }
        if (strpos($href, 'maps') !== false || strpos($t, 'direction') !== false) {
            $directions = true;
        }
    }
    ...
    foreach ($xp->query('//a/@href') as $href) {
        if (preg_match('#(facebook\.com|instagram\.com|yelp\.com|tripadvisor\.|nextdoor\.com)#i', $href->nodeValue)) {
            $social++;
        }
    }
    ...
    $add(
        'nap',
        __('Click-to-call phone number', 'sage'),
        $tel > 0 ? 'pass' : 'fail',
        12,
    ...
    $add(
        'nap',
        __('Address on the page', 'sage'),
        ($addressTag || $zipText || $schemaAddress) ? 'pass' : 'warn',
        8,
    ...
    $add(
        'schema',
        __('LocalBusiness schema', 'sage'),
        $hasLocalSchema ? 'pass' : 'fail',
        10,
    ...
    $add(
        'schema',
        __('Hours in schema', 'sage'),
        $schemaHours ? 'pass' : 'warn',
        3,
    ...
    $add(
        'map',
        __('Location in the page title', 'sage'),
        $locInTitle ? 'pass' : 'warn',
        6,
    ...
    $add(
        'map',
        __('Embedded map', 'sage'),
        $embeddedMap ? 'pass' : 'warn',
        4,
    ...
    $add(
        'map',
        __('Google Business Profile link', 'sage'),
        $gbpLink ? 'pass' : 'warn',
        5,
    ...
    $add(
        'map',
        __('Directions available', 'sage'),
        $directions ? 'pass' : 'warn',
        3,
    ...
    $add(
        'map',
        __('Mobile-ready', 'sage'),
        $viewport && $https ? 'pass' : 'warn',
        4,
    ...
    $add(
        'reviews',
        __('Review / rating schema', 'sage'),
        $schemaRating ? 'pass' : 'warn',
        6,
    ...
    $add(
        'reviews',
        __('Social / review profiles linked', 'sage'),
        $social > 0 ? 'pass' : 'warn',
        4,
