...
function rv_rest_seo(\WP_REST_Request $req)
{
    ...
    foreach ($imgs as $img) {
        ...
        if ($keyword && seocheck_has($alt, $keyword)) {
            $kwInAlt = true;
        }
    }
    ...
    // meta / indexing
    $canonical = '';
    foreach ($xp->query('//link[@rel="canonical"]/@href') as $c) {
        $canonical = trim($c->nodeValue);
    }
    $robotsNoindex = false;
    ...
    foreach ($xp->query('//meta[@name="robots"]/@content') as $r) {
        ...
        if (stripos($r->nodeValue, 'nofollow') !== false) {
            $robotsNofollow = true;
        }
    }
    ...
    $add(
        'meta',
        __('Title length 30–60 characters', 'sage'),
    ...
    $add(
        'meta',
        __('Meta description present', 'sage'),
        $metaDesc !== '' ? 'pass' : 'fail',
        9,
    ...
    $add(
        'meta',
        __('Description length 120–160', 'sage'),
    ...
    $add(
        'crawl',
        __('Links are followable', 'sage'),
        $robotsNofollow ? 'warn' : 'pass',
        4,
    ...
    $add(
        'crawl',
        __('XML sitemap referenced', 'sage'),
        $sitemapRef ? 'pass' : 'warn',
        6,
    ...
    $add(
        'content',
        __('Images described (alt)', 'sage'),
        ($imgTotal === 0 || $imgAlt / max(1, $imgTotal) >= 0.9) ? 'pass' : 'warn',
        4,
    ...
    if ($keyword) {
        $add(
            'content',
            __('Keyword in title', 'sage'),
            seocheck_has($title, $keyword) ? 'pass' : 'fail',
            9,
        ...
        $add(
            'content',
            __('Keyword in H1', 'sage'),
            seocheck_has($h1text, $keyword) ? 'pass' : 'warn',
            6,
        ...
        $add(
            'content',
            __('Keyword in meta description', 'sage'),
            seocheck_has($metaDesc, $keyword) ? 'pass' : 'warn',
            4,
        ...
        $add(
            'content',
            __('Keyword in first paragraph', 'sage'),
            seocheck_has($intro, $keyword) ? 'pass' : 'warn',
            5,
        ...
        $add(
            'content',
            __('Keyword in URL', 'sage'),
            seocheck_has($path, str_replace(' ', '-', $keyword)) || seocheck_has($path, $keyword) ? 'pass' : 'warn',
            4,
        ...
        $add(
            'content',
            __('Keyword density is natural', 'sage'),
            ($density >= 0.3 && $density <= 3.0) ? 'pass' : ($kwCount > 0 ? 'warn' : 'fail'),
            5,
        ...
        $add(
            'content',
            __('Keyword in image alt', 'sage'),
            ($imgTotal === 0 || $kwInAlt) ? 'pass' : 'warn',
            3,
    ...
    $add(
        'links',
        __('Has internal links', 'sage'),
        $band($internal >= 3, $internal >= 1),
        7,
    ...
    $add(
        'links',
        __('Descriptive anchor text', 'sage'),
    ...
    $add(
        'links',
        __('Links to relevant sources', 'sage'),
        $band($external >= 1),
        3,
    ...
    $add(
        'tech',
        __('Fast response', 'sage'),
        $band($fetch['ms'] <= 800, $fetch['ms'] <= 1500),
        6,
    ...
    $add(
        'tech',
        __('Reasonable page weight', 'sage'),
        $band($kb <= 150, $kb <= 400),
        4,
