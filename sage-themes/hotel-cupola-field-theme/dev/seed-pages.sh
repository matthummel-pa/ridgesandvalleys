...
echo "› Seeding pages for Cupola & Field Hotel…"
HOME_ID=$(mkpage home "Home" "")
AMENITIES=$(mkpage amenities "Amenities & Concierge" "template-amenities.blade.php")
AREA=$(mkpage area "The Area" "template-area.blade.php")
CONTACT=$(mkpage contact "Contact" "template-contact.blade.php")
RESERVATIONS=$(mkpage reservations "Reservations" "template-reservations.blade.php")
ROOMS=$(mkpage rooms "Rooms & Suites" "template-rooms.blade.php")

wp option update show_on_front page >/dev/null
wp option update page_on_front "$HOME_ID" >/dev/null
wp option update blogname "Cupola & Field Hotel" >/dev/null 2>&1 || true
